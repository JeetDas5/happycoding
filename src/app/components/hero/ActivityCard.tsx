...
function getColor(level: number): string {
  switch (level) {
    ...
    case 3: return 'bg-accent shadow-[0_0_6px_rgba(59,130,246,0.5)]';
  ...
  return (
    <div className="glass-card w-66 rounded-2xl p-4 relative">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs font-mono text-white bg-white/10 px-2 py-0.5 rounded-full">This Month</span>
      </div>

      {/* Count */}
      <div className="flex items-end gap-2 mb-4">
        <span className="text-4xl font-extrabold text-white font-mono leading-none">143</span>
        <span className="text-sm text-muted-foreground font-body mb-1">problems AC</span>
      </div>
      ...
      <div className="grid gap-1" style={{ gridTemplateColumns: 'repeat(7, 1fr)' }}>
        {activityData.map((level, i) => (
          <div
            key={i}
            className={`aspect-square rounded-sm transition-all hover:scale-110 ${getColor(level)}`}
